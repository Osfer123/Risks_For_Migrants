...
```{r}
#This is the Mixed effects model with Hazard_Index as outcome var and rescaled.

# Rescale the numeric predictors
df$Educ <- scale(df$Educ)
df$Aprehen <- scale(df$Aprehen)
df$hom_rt <- scale(df$hom_rt)
df$Companions <- scale(df$Companions)
df$Age <- scale(df$Age)

# Run the model again after rescaling
model_rescaled <- lmer(h_Index ~ Male + indig + Educ + eng + Married + emp + Aprehen + hom_rt_corrected + Smugg_decision + Reason_family + rel_border + alone + Age + (1| City), data = df)
...
# You can keep all predictors or filter to your variables of interest:
...
# Relabel predictor names for display
results <- results %>%
...
```{r}
# Filter: keep only significant cities + all other variables
...
```{r}
# Prepare plot data: keep only significant cities + all other variables
plot_data <- tidy_model %>%
  mutate(
    is_city = grepl("City", term),
    is_significant = p.value < 0.05
...
# Read in the CSV (replace the file path as needed)
cities <- read_csv("border_cities_coords.csv")

# Optional: make clusters as factors with names
...
# Add centroids/positions for Mexican border states
...
# Download and unzip GADM shapefile, then provide the .shp path:
...
# Read city data and prepare as before
cities <- read_csv("border_cities_coords.csv")
cluster_names <- c("Risk Level 1", "Risk Level 2", "Risk Level 3")
cities$Cluster <- factor(cities$Cluster, levels = 1:3, labels = cluster_names)
